...
It prints `wrote diagram.drawio (N nodes, M edges)` to stderr and writes a normal `.drawio` file. Continue with `drawio_validate`, export a PNG preview through `drawio_export`, self-check, review, and then perform the requested final PNG/JPEG/PDF export.
