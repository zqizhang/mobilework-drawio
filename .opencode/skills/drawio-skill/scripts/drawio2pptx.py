...
Exports each page of a (multi-page) .drawio to a PNG via the bundled HTTP
Export Server client and
...
Needs a configured Draw.io Export Server and the `python-pptx` package
...
def pages(path):
    """Page ids and names in document order."""
    try:
        root = ET.parse(path).getroot()
    except (ET.ParseError, OSError) as exc:
        sys.exit(f"error: cannot parse {path}: {exc}")
    diagrams = root.findall("diagram")
    return [(d.get("id"), d.get("name")) for d in diagrams] if diagrams else [(None, None)]
...
def export_page(drawio_file, page_id, out_png, scale):
    """Export one page to PNG through ImageExport4/export."""
    result = export_file(
        drawio_file,
        out_png,
        format="png",
...
def main():
    ...
    document_pages = pages(args.file)
    ...
    with tempfile.TemporaryDirectory() as tmp:
        for i, (page_id, name) in enumerate(document_pages, 1):
            png = os.path.join(tmp, f"page{i}.png")
            if not export_page(args.file, page_id, png, args.scale):
