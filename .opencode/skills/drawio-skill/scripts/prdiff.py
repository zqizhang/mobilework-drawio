...
base and head pages as PNGs via the bundled HTTP Export Server client, and —
for files present on both sides — chains `drawiodiff.py` -> `autolayout.py`
-> HTTP export into a
...
An unavailable Export Server degrades gracefully: the Markdown still lists
every changed file, just without images. Missing git, or `--repo` not being a
git repository, is fatal.
...
def export_png(src_drawio, out_png):
    """HTTP-export the first page of src_drawio to out_png."""
    result = export_file(src_drawio, out_png, format="png")
    return result.get("success") and os.path.exists(out_png)


def export_diff_png(base_drawio, head_drawio, out_png, tmp):
    """drawiodiff.py -> autolayout.py -> HTTP export a coloured diff PNG."""
...
def build_entry(repo, base, head, path, status, out_dir):
...
def render_markdown(entries, out_dir):
    ...
    Markdown links. "skipped" means the HTTP Export Server was unavailable.
    """
    ...
    for e in entries:
        ...
        if e.get("skipped"):
            lines.append("")
            lines.append("_Draw.io Export Server not available — images skipped._")
            continue
