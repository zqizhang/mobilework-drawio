...
Graphviz (autolayout), and a configured Draw.io HTTP Export Server.
...
def build_frame(importer, importer_args, work_path, direction, tmp):
    ...
    exported = export_file(drawio, png, format="png", scale=2)
    if not exported.get("success") or not os.path.exists(png):
        return None
