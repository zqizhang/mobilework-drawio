...
- 已绑定图表的增量修改和自动布局会先在同一 Draw.io 画布加载临时只读差异预览：可切换“修改前”“修改后”“对比”三种视图，绿色标识新增、黄色标识修改、红色标识删除或移动前位置、蓝色标识变更连线；关闭变化详情只收起面板，“取消本次修改”才会使候选失效且不会写入源文件，正式提交必须匹配预览候选哈希与基线 revision。
...
预览栏提供三个互不改变候选状态的视图：“修改前”显示原始基线，“修改后”显示不含临时标记的精确
候选，“对比”显示候选加彩色差异覆盖层并作为默认视图。右侧变化详情列出字体大小、字体颜色、填充色、
线条颜色、几何尺寸和页面背景等属性的前后值；颜色属性同时显示色块。详情面板可独立收起和重新打开，
不会取消候选。连线高亮使用独立叠加连线，不会覆盖候选版本真正的线条颜色。
...
`drawio_authorize_annotation_change`，并可绑定当前预览。用户点击“取消本次修改”、拒绝或关闭审批弹窗、
预览超过 30 分钟或图表 revision 发生变化后，候选都会失效，必须重新 dry-run；取消候选不会解决或忽略
仍然 open 的注释任务。预览状态下编辑器会停用保存和导出，Bridge
