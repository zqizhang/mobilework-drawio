...
- 已绑定图表后每轮主动检查最新revision、updatedBy和pending注释，不依赖本轮重新加载Skill。
...
- 只要当前会话已通过drawio_open或drawio_finalize绑定.drawio文件，每一轮对话开始、在决定本轮动作或回复前，都必须并发调用drawio_list_annotations(file, status="pending")和drawio_get_state：前者检查未完成注释，后者检查最新revision、updatedBy及人工或外部版本变动。此规则属于主Agent契约，不依赖本轮是否重新加载Skill；用户只说“嗯”“继续”“好了吗”等简短回复时也不得跳过。
...
- 合并判断每轮双探测结果：updatedBy=editor/external/restore表示画布版本发生人工、外部或恢复变动，但是否询问仍以注释requiresConfirmation为准；只读取pending任务并跳过resolved和ignored，fresh任务直接进入计划和审批，stale任务先说明变化并询问，随后都必须dry-run并公开计划、稳定ID和范围，再把preview_id传给drawio_authorize_annotation_change触发当前session的写前审批；用户未看图并批准前不得修改，禁止先改后问。
- 若用户本轮有其它明确任务，完成该任务后必须在同一轮重新并发调用drawio_list_annotations(file, status="pending")和drawio_get_state并重新计算freshness，不能沿用本轮开始时的结果；最终回复前再次列出pending注释，除非用户明确要求暂不处理、注释需要确认或工具失败，否则不得遗留requiresConfirmation=false的fresh注释。
...
- 仅当drawio_finalize返回shouldOpenBrowser=true时用openwork_browser_open_url打开内置浏览器；已有编辑器连接时不得重复打开或刷新。Agent继续写入前重新读取revision。
...
- 返回任务理解、使用的Skill和工具、目标与导出文件、页面和语义结构、最新revision与updatedBy、pending注释处理状态、验证状态、评分、差异、备份、错误或警告、能力限制和剩余风险。
...
- 每次创建或修改成功后必须产生同名PNG；仅当drawio_finalize返回shouldOpenBrowser=true时才通过MobileWork工具openwork_browser_open_url，并传入url=openUrl、provider="builtin"打开openUrl。
...
- 已绑定会话每轮开始必须完成pending注释与最新revision/updatedBy双探测；有其它明确任务时完成后重新探测，最终回复前不得遗留可直接执行的fresh注释。
