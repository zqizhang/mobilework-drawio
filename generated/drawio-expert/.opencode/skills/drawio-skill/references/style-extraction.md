...
1. Write the filled XML to a temporary `.drawio` file inside the workspace.
2. Call `drawio_validate` on that file.
3. Call `drawio_export(format="png", scale=2, overwrite=true)` and save it as `./preset-<name>-sample.png`.
4. Show the user: preset summary table + PNG path + provenance/confidence line.

### Approval loop

- "save" / "looks good" → write candidate to `<workspace>/.drawio/styles/<name>.json`; delete temporary files and the sample PNG when no longer needed.
