...
1. `<workspace>/.drawio/styles/<name>.json` — workspace presets.
...
Only workspace presets can have `"default": true`. When the user says *"make `<built-in-name>` my default"*, copy the built-in JSON to `<workspace>/.drawio/styles/<name>.json` first, then set `default: true` on the copy — leave the shipped built-in untouched.
...
3. **Normalize and build candidate.** Convert the user-provided preset name to lowercase. Use this normalized name for ALL file paths in this flow. Build the candidate preset JSON in a temporary workspace directory. Do **not** save to `.drawio/styles/<name>.json` yet.
4. **Render a sample** using the sample-diagram skeleton in `references/style-extraction.md`, parameterized by the candidate preset. Validate it, then call `drawio_export(format="png", scale=2, overwrite=true)` to produce `./preset-<name>-sample.png`.
5. **Show the user:**
...
6. **Wait for approval:**
   - "save" / "looks good" → write candidate to `<workspace>/.drawio/styles/<name>.json`. Create the workspace directory if it doesn't exist. Delete temporary files and sample PNG.
...
| XML parse fails | Stop; report the parse error; repair the XML or use `drawio_open` for manual correction in the built-in browser. |
...
|---|---|
| "list my styles", "what styles do I have", "show me my style presets" | Read `<workspace>/.drawio/styles/` and `<this-skill-dir>/styles/built-in/`. Print a table: `name`, `location` (workspace/built-in), `source.type`, `confidence`, `default` flag. Built-ins shadowed by a workspace preset are marked so. |
| "show my `<name>` style", "what's in `<name>`" | Print the preset JSON (pretty-printed) + a one-line summary (source, confidence, is-default). |
| "make `<name>` the default", "set `<name>` as default" | If `<name>` is a workspace preset: set `default: true` on it; clear `default` on any other workspace preset that had it; save both files. If `<name>` is a built-in: copy `<this-skill-dir>/styles/built-in/<name>.json` → `<workspace>/.drawio/styles/<name>.json` first, then set `default: true` on the copy. Never mutate the shipped built-in. |
| "remove default", "unset default" | Clear `default: true` from whichever user preset has it. |
| "delete `<name>`", "remove `<name>`" | Confirm first, then delete `<workspace>/.drawio/styles/<name>.json`. Refuse to delete shipped built-ins. |
| "rename `<a>` to `<b>`" | Rename the file inside `<workspace>/.drawio/styles/`, then update its `name` field. If `<a>` is a built-in, offer to copy it into the workspace first. |
