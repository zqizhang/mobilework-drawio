...
| a `.drawio` | a shareable interactive viewer | `drawiohtml` (→ PNG-based HTML: pan/zoom/tabs) |
| a `.drawio` | a written description | `explain` (→ Markdown) |
| a `.drawio` | a slide deck | `drawio2pptx` (→ PPTX) |
| an existing `.svg` | animated data-flow | `svgflow` (does not convert `.drawio`) |
...
- **`buildup.py`** — reveal ONE diagram's cells in dependency order (topological over its edges) → self-contained HTML player (embedded PNG frames, play/pause/step/scrub); optional `--gif`. Frames use the bundled HTTP exporter.
...
- **`drawiohtml.py`** — → a self-contained **PNG-based interactive HTML viewer** with tabs, drag-pan, wheel-zoom, and fit controls. The generated file needs no server; SVG node search and drill-down are not available.
...
- **`svgflow.py`** — add marching-ants edge animation to an **existing SVG**. It does not export `.drawio` to SVG.
...
- For interactive editing, use `drawio_open`, then call MobileWork's `openwork_browser_open_url` tool with `url=openUrl` and `provider="builtin"` when opening is required.
