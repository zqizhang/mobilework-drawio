...
every frame so nothing jumps around as cells appear. PNG frames use the
bundled HTTP Export Server client; no Draw.io Desktop installation is needed.
...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        for k in range(n_total):
            ...
            exported = export_file(src, png_path, format="png", scale=2)
            if not exported.get("success") or not os.path.exists(png_path):
                sys.stderr.write(
                    f"warning: step {k + 1}/{n_total} export failed — "
                    f"{exported.get('message', 'unknown error')}\n"
                )
                continue
    ...
    if not frames:
        sys.exit("error: no frames exported; check DRAWIO_EXPORT_URL and the Export Server")
