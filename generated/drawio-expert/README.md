...
- 批注按图表文件持久化，换对话后打开同一图表仍可继续处理；正式修改前先dry-run并调用drawio_authorize_annotation_change取得当前session的一次性授权。
...
- 需要自动优化时先dry-run调用drawio_polish；存在活动批注时，只有取得全图范围审批后才能正式写入并保留备份。
