...
- 已绑定图表的patch和polish先dry-run，在同一画布以绿/黄/红/蓝显示新增、修改、删除或原位置、变更连线；预览不写入源文件，普通任务看图后通过drawio_authorize_preview审批，弹窗允许后立即提交获批候选，无需再发送文字确认。
- 批注按图表文件持久化，换对话后打开同一图表仍可继续处理；正式修改前先dry-run，把preview_id传给drawio_authorize_annotation_change取得当前session的一次性授权。
- 批注范围支持选区、关联连线、周边布局和整个图表；全图范围会额外确认，只覆盖当前文件且跨页稳定ID使用`pageId:cellId`。
- 批注列表支持按待处理、未完成、已过时、已完成和已忽略筛选；用户可手动完成、忽略或重新打开，已完成和已忽略批注不会再交给Agent处理。
- 注释修改不得越过用户选择的范围；确需越界时先说明原因并通过审批弹窗申请更宽范围，未批准则停止。
- 每次生成或修改成功后必须调用drawio_finalize，自动校验、评分、导出同名PNG并返回openUrl。
- 需要自动优化时先dry-run调用drawio_polish并在画布查看预览；存在活动批注时，只有取得全图范围审批后才能正式写入并保留备份。
