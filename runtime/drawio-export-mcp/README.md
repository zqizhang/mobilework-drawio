...
- Python >= 3.12
- `uv` 包管理器（推荐）或 `pip`
- 可访问的 Draw.io Export Server；包内占位默认值为本机 `127.0.0.1:18765`
...
| 变量 | 默认值 | 说明 |
|------|--------|------|
| `DRAWIO_EXPORT_URL` | `http://127.0.0.1:18765/ImageExport4/export` | Export Server地址；部署时应显式配置 |
...
"mcp": {
  "drawio-export": {
    ...
    "environment": {
      "DRAWIO_EXPORT_URL": "{env:DRAWIO_EXPORT_URL}",
...
│       ├── service.py          # MCP与Skill脚本共享的文件导出流程
│       ├── cli.py              # 同一导出服务的命令行适配器
