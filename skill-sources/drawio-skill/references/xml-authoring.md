...
**Edge style rules:**
- **Animated connectors:** add `flowAnimation=1;` to an edge style when the interactive Draw.io editor should show motion. Static PNG/JPEG/PDF exports may not preserve animation. Example: `style="edgeStyle=orthogonalEdgeStyle;flowAnimation=1;rounded=1;..."`
...
- **Interactive obstacle routing:** the HTTP exporter does not recompute connector routes. Keep the orthogonal rules above; if a dense diagram still has crossings, use `drawio_open` in MobileWork's built-in browser for manual rerouting, then read the latest revision before the Agent continues.
