...
A skill that turns natural-language descriptions into `.drawio` XML and exports them to PNG / JPEG / PDF via a **Draw.io Export MCP Server**. It can also turn an **existing codebase** (Python / JS-TS / Go / Rust), **Terraform / Kubernetes / docker-compose infrastructure**, or a **SQL schema** into an auto-laid-out diagram. Works with **Claude Code, Cursor, Copilot, OpenClaw, Codex, Autohand Code, Hermes**, and any agent compatible with the [Agent Skills](https://agentskills.io) format.

> **Note:** As of v3.0.0, PNG / JPEG / PDF export has been migrated from the Draw.io Desktop CLI to the Draw.io Export MCP. The Draw.io Desktop app is no longer required for these formats. SVG export, Mermaid→draw.io conversion, and ELK layout still require the desktop CLI (see [Current Limitations](#current-limitations)).
...
### 1. Set up the Draw.io Export MCP Server

PNG, JPEG, and PDF export now go through a local MCP server that communicates with a remote Draw.io Export Server. The Draw.io Desktop app is **no longer required** for these formats.

1. Install and configure the `drawio-export-mcp` server (see its own README).
2. Add it to your agent's MCP configuration (e.g. `.opencode/opencode.jsonc` in OpenCode):

```jsonc
"mcp": {
  "drawio-export": {
    "type": "local",
    "command": ["/path/to/python", "-m", "drawio_export_mcp.server"],
    "environment": {
      "DRAWIO_EXPORT_URL": "http://192.168.1.210:18765/ImageExport4/export",
      "DRAWIO_WORKSPACE_ROOT": "/path/to/workspace"
    },
    "enabled": true
  }
}
...
3. Verify the MCP tools are discoverable: `drawio_export`, `drawio_validate`, `drawio_health_check`.

### 2. (Optional) Install the draw.io desktop CLI for SVG / Mermaid / ELK

SVG export, Mermaid→`.drawio` conversion, and the ELK `--layout` pass still need the desktop CLI. Skip this if you only need PNG / JPEG / PDF.
...
Behind the scenes: **check dependencies → plan layout → generate `.drawio` XML → `drawio_validate` → export draft PNG via `drawio_export` → self-check + auto-fix** (up to 2 rounds) → **show to user → 5-round feedback loop** until approved → **final export via `drawio_export`**.

### Current Limitations

The following features have **not yet been migrated** from the Draw.io Desktop CLI to the MCP backend:

| Feature | Status | Fallback |
|---------|--------|----------|
| PNG export | ✅ MCP (`drawio_export`, format="png") | — |
| JPEG export | ✅ MCP (`drawio_export`, format="jpeg") | — |
| PDF export | ✅ MCP (`drawio_export`, format="pdf") | — |
| XML validation | ✅ MCP (`drawio_validate`) | — |
| Health check | ✅ MCP (`drawio_health_check`) | — |
| SVG export | ❌ Not yet supported | Desktop CLI or diagrams.net viewer |
| Mermaid→draw.io | ❌ Not yet supported | Desktop CLI (≥ v30) |
| ELK `--layout` | ❌ Not yet supported | `autolayout.py` (Graphviz) |
| Embed XML (`-e`) | ⚠️ Partial (`embed_xml` param) | Desktop CLI for editable PNG |

The `.drawio` source file is always preserved, so users can open it in the draw.io desktop app for any format or feature not yet covered by MCP.
