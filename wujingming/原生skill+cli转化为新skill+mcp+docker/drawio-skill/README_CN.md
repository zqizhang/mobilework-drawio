...
一个把自然语言描述变成 `.drawio` XML，并通过 **Draw.io Export MCP Server** 导出为 PNG / JPEG / PDF 的技能。它还能把**现有代码库**（Python / JS-TS / Go / Rust）、**Terraform / Kubernetes / docker-compose 基础设施**或 **SQL schema** 转成自动布局的图表。支持 **Claude Code、Cursor、Copilot、OpenClaw、Codex、Autohand Code、Hermes** 等任何兼容 [Agent Skills](https://agentskills.io) 规范的 agent。

> **注意：** 从 v3.0.0 起，PNG / JPEG / PDF 导出已从 Draw.io Desktop CLI 迁移至 Draw.io Export MCP。这三种格式不再需要安装 Draw.io Desktop。SVG 导出、Mermaid→draw.io 转换和 ELK 布局仍需要桌面版 CLI（参见[当前限制](#当前限制)）。
