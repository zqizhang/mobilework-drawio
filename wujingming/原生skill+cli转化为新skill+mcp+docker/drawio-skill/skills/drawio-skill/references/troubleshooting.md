...
Read this when something looks wrong in the output (rendering, export, layout, edges) or when an MCP/CLI invocation fails. Most rows have a one-line fix.

## MCP Export Issues (v3.0.0+)

| Problem | Action |
|---------|--------|
| `drawio_export` returns `EXPORT_SERVER_UNREACHABLE` | Call `drawio_health_check` to diagnose. The remote Export Server may be down or network is blocked. |
| `drawio_export` returns `EXPORT_SERVER_TIMEOUT` | The diagram may be too complex or the server overloaded. Try reducing scale or splitting into multiple pages. |
| `drawio_export` returns `INVALID_DRAWIO_XML` | Call `drawio_validate` to get detailed XML structure errors. Fix the XML and retry. |
| `drawio_export` returns `OUTPUT_ALREADY_EXISTS` | Set `overwrite=true` or choose a different `output_path`. |
| `drawio_export` returns `UNSUPPORTED_FORMAT` | Only `png`, `jpeg`, `pdf` are supported via MCP. SVG is not yet available. |
| `drawio_validate` returns validation errors | Check the returned `error_code` and `message`. Common causes: XML parse error, wrong root element, missing `<diagram>` elements. |
| Export succeeds but content looks wrong in PNG | The export server may have rendered a blank or error diagram. Check the `.drawio` XML is well-formed with `drawio_validate`. |
| `drawio_health_check` deep test fails | The export server returned content that doesn't look like a valid PNG. Check server configuration. |
| Need SVG / Mermaid→drawio / ELK layout | These features still require the Draw.io Desktop CLI. MCP does not yet support them. See SKILL.md "当前MCP能力边界". |

## CLI Export Issues (draw.io Desktop — for SVG / Mermaid / ELK only)
